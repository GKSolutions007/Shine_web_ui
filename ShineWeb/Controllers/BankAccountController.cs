...
namespace ShineWeb.Controllers
{
    public class BankAccountController : Controller
    {
        ...
        public ActionResult Index(string Name, string strFormID, string TypeID, string TranID, string isDepend = "0", string ParMID = "", string ParFFID = "")
        {
            ...
            else
            {
                ...
                string ParentMenuID = "0", ParentFormFieldID = ""; ;
                if (!string.IsNullOrEmpty(ParMID))
                {
                    ParentMenuID = clsEncryptDecrypt.Decrypt(ParMID);
                }
                if (!string.IsNullOrEmpty(ParFFID))
                {
                    ParentFormFieldID = ParFFID;
                }
                ViewData["isDepend"] = isDepend;
                ViewData["ParentMenuID"] = ParentMenuID;
                ViewData["ParentFormFieldID"] = ParentFormFieldID;
                ...
                string url = "";
                DataRow[] drrSMGroup = dtPermission.Select("MenuID = 2");// Bank

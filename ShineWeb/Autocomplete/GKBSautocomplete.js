function PaginatedAutocomplete_old(options) {
    ...
    // ── setActive: ONLY highlights + scrolls. No loadNext() here. ──────────
    function setActive(idx) {
        ...
        state.activeIdx = idx;
        if (idx >= 0) {
            scrollIntoView($els.get(idx));   // .get() → raw DOM node
        }
    }
    ...
    // ── loadNext: optional afterLoad callback fires AFTER items are appended ─
    function loadNext(afterLoad) {
        if (state.loading || !state.hasMore) {
            // if already loaded everything and callback given, still fire it
        ...
        setTimeout(function () {
            ...
            slice.forEach(function (item) {
                var $li = $("<li>").css({
                    ...
                    highlight(item.label, state.query) +
                    '</div>' +
            ...
            // fire callback AFTER items are in the DOM
    ...
    function reset(query) {
        ...
        loadNext();  // load first page, no callback needed here
    }
    ...
    // ── Scroll → next page (mouse scroll only) ─────────────────────────────
    $("#" + listId).on("scroll", function () {
        if (this.scrollTop + this.clientHeight >= this.scrollHeight - 40) {
            loadNext();   // no callback — scroll load doesn't move cursor
        }
    });
    ...
    $input.off("keydown.pac").on("keydown.pac", function (e) {
        ...
        if (e.key === "ArrowDown") {
            ...
            var next = state.activeIdx + 1;   // desired next index

            if (next < total) {
                // ✅ next item already in DOM → move one step, no load needed
                setActive(next);

            } else if (state.hasMore && !state.loading) {
                // ✅ exhausted loaded items, more exist → load page then move cursor
                loadNext(function () {
                    setActive(next);   // 'next' is exactly the first index of new page
                });

            }
            // if state.loading === true → key press ignored, prevents double-load
            return;
        }
    ...
    var debounce;
    $input.off("input.pac").on("input.pac", function () {
    ...
    $input.off("focus.pac").on("focus.pac", function () {
        if (!$input.val()) reset("");
        $GKBSdropdown.show();
    });

    $input.off("blur.pac").on("blur.pac", function () {
        setTimeout(function () { $GKBSdropdown.hide(); }, 180);
    });
